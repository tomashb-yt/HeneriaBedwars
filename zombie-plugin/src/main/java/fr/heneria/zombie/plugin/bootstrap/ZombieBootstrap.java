...
public final class ZombieBootstrap {
  ...
  public void start() throws Exception {
    ...
    fr.heneria.zombie.plugin.editor.ManagedMapWorldService mapWorldManagement =
        new fr.heneria.zombie.plugin.editor.ManagedMapWorldService(
            plugin.getServer().getWorldContainer().toPath(), templates, ioExecutor, mainThread);
    ...
    new fr.heneria.zombie.plugin.gui.MapMenuModule(
            guiRegistry,
            guiActions,
            guiConfigurations,
            guiService,
    ...
    registerListeners(
        ...
        new EditorPlacementListener(
            ...
            Clock.systemUTC(),
            mainThread),
        new EditorSessionListener(editorService, editorItems, mapVisualizations));
    registerCommand(
        configurations,
        messages,
        templates,
    ...
    registerMapEditorCommand(mapValidator, mapWorldManagement, mainThread);
  ...
  public void stop() {
    if (editorService != null) {
      ...
      editorService = null;
      editorItems = null;
      if (mapVisualizations != null) {
        mapVisualizations.clearAll();
        mapVisualizations = null;
      }
    }
    if (guiService != null) {
      guiService.shutdown();
  ...
  private void registerPlayerGuiCommand(GuiService guiService) {
    ...
    command.setExecutor(
        (sender, ignored, label, arguments) -> {
          if (!(sender instanceof org.bukkit.entity.Player player)) {
          ...
          } else if (!player.hasPermission("zombies.menu.player")) {
            player.sendMessage(
            ...
            boolean admin =
                arguments.length > 0
                    && arguments[0].equalsIgnoreCase("admin")
                    && player.hasPermission("zombies.menu.admin");
            guiService.openHome(
                player,
                new fr.heneria.zombie.plugin.gui.GuiId(admin ? "admin-main" : "player-main"));
          }
          return true;
        });
  }
  ...
  private void registerMapEditorCommand(
      ...
      fr.heneria.zombie.plugin.editor.ManagedMapWorldService worldManagement,
      PaperMainThreadExecutor mainThread) {
